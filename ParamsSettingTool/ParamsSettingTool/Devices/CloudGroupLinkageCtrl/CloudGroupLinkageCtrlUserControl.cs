...
///5、云群控器设备类型增加新基点IP和端口、是否具有新基点属性HasNewBenchmark  CloudEntranceInfo 
...
namespace ITL.ParamsSettingTool.ParamsSettingTool.Devices.CloudGroupLinkageCtrl
{
    public delegate void GetCloudFloorTable(FloorTableInfo data);

    public partial class CloudGroupLinkageCtrlUserControl : GeneralDeviceUserControl
    {
        public CloudGroupLinkageCtrlUserControl()
        {
            InitializeComponent();
        }
        #region 字段常量定义
        ...
        private CloudGroupCloudLinkageInfo f_CurrentElevatorInfo = null;  //当前操作的设备信息
        ...
        protected override void InitUIOnLoad()
        {
            ...
            this.DevType = DeviceType.CloudGroupCloudLink;
        ...
        protected override void InitDevicesGridView(GridView grdView)
        {
            ...
            ////线下物联平台服务器Ip
            //this.AddOneGridViewColumn(grdView, EI_SERVER_IP, EI_SERVER_IP_ALIAS, 150);

            ////线下物联平台服务器端口
            //this.AddOneGridViewColumn(grdView, EI_SERVER_PORT, EI_SERVER_PORT_ALIAS, 150);
            ...
            //版本号
            this.AddOneGridViewColumn(grdView, VERSION, VERSION_ALIAS, 200);
            //项目编号
        ...
        public void InitGrid()
        {
            ...
            GridBand bandTerminalNumSlave1 = grdFloorTable.Bands.AddBand("第一副操纵盘端子号");

            GridBand bandTerminalNumSlave2 = grdFloorTable.Bands.AddBand("第二副操纵盘端子号");
        ...
        public static explicit operator CloudGroupLinkageCtrlUserControl(Form v)
        {
        ...
        private void AddOneDeviceToUI(CloudGroupCloudLinkageInfo deviceInfo)
        {
            ...
            if (rows.Length > 0)
            {
                ...
                try
                {
                    ...
                    rows[0][GROUP_DEV_TYPE] = deviceInfo.GroupDevType;
                    rows[0][IS_COMM_DEV] = deviceInfo.IsCommGroupDev;
                    rows[0][MAIN_GROUP_DEV_IP] = deviceInfo.MainGroupDevIP;
            ...
            {
                f_DevicesDataTable.Rows.Add(deviceInfo.DevId, 
                    deviceInfo.DevMac, 
                    deviceInfo.DevIp,
                    ...
                    deviceInfo.GroupDevType, 
                    deviceInfo.IsCommGroupDev, 
                    deviceInfo.MainGroupDevIP,
        ...
        protected override void AnalysisRecvDataEx(string strCmdStr)
        {
            ...
            if (devType != DeviceType.CloudGroupCloudLink)
            {
                return;
            }
            //根据命令字，解析对应命令
            ...
            else if (strCmdWord == AppConst.CMD_WORD_SET_CLOUD_GROUPLIKE_PARAMS)
            {
                //判断报文长度
        ...
        private void AnalysisSearchDevicesRecvData(string strCmdStr)
        {
            ...
            CloudGroupCloudLinkageInfo deviceInfo = new CloudGroupCloudLinkageInfo();
            ...
            if (strCmdReport.Length > 98)
            {
                ...
                string SubVerSionStr = StrUtils.CopySubStr(deviceInfo.VerSion, 0, 7);

                if (SubVerSionStr == COMMON_GROUP_DEV)
                {
                    deviceInfo.IsCommGroupDev = true;
                    deviceInfo.GroupDevType = "普通群控器";
                    //韦将杰 2020-04-27 不给搜索到云群控器
                    return;
                } else
                {
                    deviceInfo.IsCommGroupDev = false;
                    deviceInfo.GroupDevType = "云群控器";
                }

                //2020-04-28 主群控器ip起始位置，82位 + 版本号长度(主群控器ip目前是保留字段)
                int indexMainIPStart = 82 + f_VersionLength;
                deviceInfo.MainGroupDevIP = CommonUtils.GetIPByHex(StrUtils.CopySubStr(strCmdReport, indexMainIPStart, 8));

                //韦将杰 2020-04-26 增加新基点IP和接口解析
                //华为新基点IP
                //
                
                int indexNewBenchmark = indexMainIPStart + 8;
                int hasNewBenchmarkLenght = indexNewBenchmark + 8 + 4;
                if (strCmdReport.Length >= hasNewBenchmarkLenght)
                {
                    deviceInfo.NewBenchmarkIP = CommonUtils.GetIPByHex(StrUtils.CopySubStr(strCmdReport, indexNewBenchmark, 8));
                    //华为新基点端口          
                    deviceInfo.NewBenchmarkPort = StrUtils.StrToIntDef(StrUtils.CopySubStr(strCmdReport, indexNewBenchmark + 8, 4), 0, 16).ToString();
        ...
        private void AnalysisDownParams(string strCmdStr)
        {
            ...
            switch (intStatus)
            {
                case 0x00:
                    {
                        //this.AddOneDeviceToUI(f_CurrentElevatorInfo);
       ...
       /// </summary>
        ...
        {
            CloudGroupCloudLinkageInfo elevatorInfo = GetDeviceInfoBeforeDown();
        ...
        /// 下载设备参数.
        /// </summary>
        /// <param name="elevatorInfo">云群控器.</param>
        private void DownBasicParams(CloudGroupCloudLinkageInfo elevatorInfo)
        {
            ...
            string writeReport = this.GetWriteReport(elevatorInfo.DevId, AppConst.CMD_WORD_SET_CLOUD_GROUPLIKE_PARAMS, strWriteCmd);
        ...
        /// <returns></returns>
        private string GetBasicParamsWriteCmdStr(CloudGroupCloudLinkageInfo deviceInfo)
        {
            //协议版本，目前固定为02
            string strCmdStr = "02";
            //DHCP标志
            ...
            //线下物联平台服务器地址
            //strCmdStr += CommonUtils.GetHexByIP(elevatorInfo.EIServerIp);
            //线下物联平台服务器端口
            //strCmdStr += StrUtils.IntToHex(elevatorInfo.EIServerPort, 4);
            //联动器端口
            //strCmdStr += StrUtils.IntToHex(elevatorInfo.LinkagePort, 4);
            ...
            strCmdStr += !deviceInfo.IsCommGroupDev ? "00000000" : CommonUtils.GetHexByIP(deviceInfo.MainGroupDevIP);
        ...
        private CloudGroupCloudLinkageInfo GetDeviceInfoBeforeDown()
        {
            if (!this.CheckUIValid())
            {
                return null;
            }
            CloudGroupCloudLinkageInfo deviceInfo = this.GetDeviceInfoByFocusRow();
            if (deviceInfo == null)
            {
                return null;
            }
            ...
            deviceInfo.MainGroupDevIP = this.edtMainGroupDevIP.Text.Trim();
        ...
        private CloudGroupCloudLinkageInfo GetDeviceInfoBeforeDownV2(CloudGroupCloudLinkageInfo deviceInfo)
        {
            if (!this.CheckUIValid())
            {
                return null;
        ...
        protected override void ExcuteDoubleClick()
        {
            ...
            try
            {
                ...
                //并且设
                //设备为云群控器，则项目编号可编辑
                edtProjectNo.Enabled = edtProjectNo.Text.Trim() =="00000000" && (! deviceInfo.IsCommGroupDev);

                //群控器为普通群控器时主群控器相关才可见
                layoutControlItemMainGroupDev.ContentVisible = deviceInfo.IsCommGroupDev;
                liMainGroupDevIP.ContentVisible = deviceInfo.IsCommGroupDev;

                //如果主群控器IP为0.0.0.0.则说明设备为主群控器
                if (deviceInfo.MainGroupDevIP == "0.0.0.0")
                {
                ...
                //主群控器IP
                edtMainGroupDevIP.Text = deviceInfo.MainGroupDevIP;
        ...
        /// <summary>
        /// 取出当前焦点行的数据，存储到结构体elevatorInfo中
        /// </summary>
        /// <returns></returns>
        private CloudGroupCloudLinkageInfo GetDeviceInfoByFocusRow()
        {
        ...
        /// 快速设置
        /// </summary>
        private void QuickSetInLocal(CloudGroupCloudLinkageInfo elevatorInfo)
        {
            //如果为云端下载的数据不执行快速设置
            if (IsCloudDownFloor)
            {
        ...
        private void DownFloorTables(CloudGroupCloudLinkageInfo elevatorInfo)
        {
            try
            {
                string strCmdWord = string.Empty;
                string strWriteCmd = string.Empty;
                this.FloorTableWriteCmdStr(elevatorInfo, ref strWriteCmd, ref strCmdWord);
                //string WriteReport = this.GetWriteReport(elevatorInfo.DevId, "5A30", "");//报文
                strWriteCmd = strWriteCmd.ToUpper();
                string WriteReport = this.GetWriteReport(elevatorInfo.DevId, "5A30", strWriteCmd);//报文
        ...
        private void FloorTableWriteCmdStr(CloudGroupCloudLinkageInfo elevatorInfo, ref string WriteCmd, ref string cmdWord)
        {
        ...
        /// </summary>
        /// <param name="elevatorInfo"></param>
        private void ReadFloorTables(CloudGroupCloudLinkageInfo elevatorInfo)
        {
            try
            {
                string strCmdWord = string.Empty;
        ...
        /// <param name="cmdWord"></param>
        private void GetFloorTableReadCmdStr(CloudGroupCloudLinkageInfo elevatorInfo, ref string readCmd, ref string cmdWord)
        {
        ...
        private void GrdFloorTable_ValidateFloorEditor(object sender, BaseContainerValidateEditorEventArgs e)
        {
            ...
            var tmp = string.Format("与权限标识{0}的楼层相同，是否交换？", relUIExchange.FloorNo);
        ...
        private void QuickSetFloorTable()
        {
            ...
            if (!string.IsNullOrEmpty(f_StratAuthFlag))
            {
                ...
                for (; Dev_start <= Dev_End; Dev_start++)
                {
                    ...
                    {
                        //端子号
                        SelectView.SetRowCellValue(Dev_start - 1, nameof(FloorRelationUIObject.FloorTerminalNo), listQuickSet_DevNo[Dev_start - 1]);
                    }
                    if (f_QuickSetInfo_CheckFloor)
                    {
                        //端子号
                        SelectView.SetRowCellValue(Dev_start - 1, nameof(FloorRelationUIObject.CheckFloor), listQuickSet_DevNo[Dev_start - 1]);
                    }
                    if (f_QuickSetInfo_FloorName)
                    {
                        //端子号
                        SelectView.SetRowCellValue(Dev_start - 1, nameof(FloorRelationUIObject.FloorName), listQuickSet_DevNo[Dev_start - 1]);
                    }
                    if (f_QuickSetInfo_TerminalNumSlave1)
                    {
                        //端子号
                        SelectView.SetRowCellValue(Dev_start - 1, nameof(FloorRelationUIObject.TerminalNumSlave1), listQuickSet_DevNo[Dev_start - 1]);
                    }
                    if (f_QuickSetInfo_TerminalNumSlave2)
                    {
                        //端子号
                        SelectView.SetRowCellValue(Dev_start - 1, nameof(FloorRelationUIObject.TerminalNumSlave2), listQuickSet_DevNo[Dev_start - 1]);
                    }
                    TmpSet++;
                }
                if (Dev_End == 112)
                {
                    SelectView.SetRowCellValue(Dev_start, nameof(FloorRelationUIObject.FloorName), listQuickSet_KeyName[Dev_start - 1]);
                    SelectView.SetRowCellValue(Dev_start, nameof(FloorRelationUIObject.CheckFloor), listQuickSet_CheckFloor[Dev_start - 1]);
                    SelectView.SetRowCellValue(Dev_start, nameof(FloorRelationUIObject.FloorTerminalNo), listQuickSet_DevNo[Dev_start - 1]);
                }
                f_QuickSetInfo_FloorTerminalNo = false;
                f_QuickSetInfo_CheckFloor = false;
                f_QuickSetInfo_FloorName = false;
        ...
        {
            var data = (List<FloorRelationUIObject>)gridControl_FloorTable.DataSource;
            ...
            if (data == null)
            {
                data = new List<FloorRelationUIObject>();
            }
            //data.Remove(item);
            data.Add(addItem);
            gridControl_FloorTable.DataSource = data;
        ...
        private List<CloudGroupCloudLinkageInfo> GetDeviceInfoBySelectData()
        {
            List<CloudGroupCloudLinkageInfo> cloudElevatorInfoList = new List<CloudGroupCloudLinkageInfo>();
        ...
        protected void DownParamsV2(CloudGroupCloudLinkageInfo cloudElevatorInfo)
        {
            CloudGroupCloudLinkageInfo elevatorInfo = GetDeviceInfoBeforeDownV2(cloudElevatorInfo);
            if (elevatorInfo == null)
            {
                return;
            }
